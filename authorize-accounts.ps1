...
$MQL5_BASE = "$env:APPDATA\MetaQuotes\Terminal\D0E8209F77C8CF37AD8BF550E51FF075\MQL5\Experts\EA Budak Ubat"
$MQL5_ROOT = "$env:APPDATA\MetaQuotes\Terminal\D0E8209F77C8CF37AD8BF550E51FF075\MQL5"
...
if ($Platform -eq "ALL" -or $Platform -eq "MT5") {
    Prepend-To-TxtList "$MQL5_BASE\v1.67 Authorized Account List.txt"
    Prepend-To-TxtList "$DESKTOP_MT5_REPO\v1.67 Authorized Account List.txt"
    Prepend-To-TxtList "$PUBLIC_REPO\v1.67 Authorized Account List.txt"
    Prepend-To-TxtList "$MQL5_BASE\v1.66 Authorized Account List.txt"
    Prepend-To-TxtList "$DESKTOP_MT5_REPO\v1.66 Authorized Account List.txt"
    Prepend-To-TxtList "$PUBLIC_REPO\v1.66 Authorized Account List.txt"
...
if ($Platform -eq "ALL" -or $Platform -eq "MT5") {
    Prepend-To-MQSource "$MQL5_BASE\EA - Budak Ubat v1.67 - MT5 - .mq5"
    Prepend-To-MQSource "$DESKTOP_MT5_REPO\EA - Budak Ubat v1.67 - MT5 - .mq5"
    Prepend-To-MQSource "$MQL5_BASE\EA - Budak Ubat v1.66 - MT5 - .mq5"
    Prepend-To-MQSource "$DESKTOP_MT5_REPO\EA - Budak Ubat v1.66 - MT5 - .mq5"
...
if ($Platform -eq "ALL" -or $Platform -eq "MT5") {
    # 0. EA Budak Ubat MT5 v1.67 (Latest Flagship)
    $mq5_167 = "$MQL5_BASE\EA - Budak Ubat v1.67 - MT5 - .mq5"
    if (Test-Path $mq5_167) {
        Write-Host "  Compiling MT5: EA Budak Ubat v1.67..." -ForegroundColor DarkCyan
        Start-Process -FilePath $MT5_COMPILER -ArgumentList "/compile:`"$mq5_167`" /log:`"$MQL5_BASE\compile_v167.log`"" -PassThru -Wait -NoNewWindow
        $bin167 = "$MQL5_BASE\EA - Budak Ubat v1.67 - MT5 - .ex5"
        if (Test-Path $bin167) {
            Copy-Item $bin167 "$MQL5_BASE\EA - Budak Ubat v1.67 - MT5 - $dateSuffix.ex5" -Force
            Copy-Item $bin167 "$PUBLIC_REPO\EA - Budak Ubat v1.67 - MT5 - $dateSuffix.ex5" -Force
            Copy-Item $bin167 "$PUBLIC_REPO\EA - Budak Ubat v1.67 - MT5 - .ex5" -Force
            Copy-Item $bin167 "$DESKTOP_MT5_REPO\EA - Budak Ubat v1.67 - MT5 - $dateSuffix.ex5" -Force
            Copy-Item $bin167 "$DESKTOP_MT5_REPO\EA - Budak Ubat v1.67 - MT5 - .ex5" -Force
            Copy-Item $mq5_167 "$DESKTOP_MT5_REPO\EA - Budak Ubat v1.67 - MT5 - .mq5" -Force
            Write-Host "  [OK] EA Budak Ubat MT5 v1.67 compiled!" -ForegroundColor Green
        }
    }

    # 0.1. EA Budak Ubat MT5 v1.66
    $mq5_166 = "$MQL5_BASE\EA - Budak Ubat v1.66 - MT5 - .mq5"
    if (Test-Path $mq5_166) {
        Write-Host "  Compiling MT5: EA Budak Ubat v1.66..." -ForegroundColor DarkCyan
        Start-Process -FilePath $MT5_COMPILER -ArgumentList "/compile:`"$mq5_166`" /log:`"$MQL5_BASE\compile_v166.log`"" -PassThru -Wait -NoNewWindow
        $bin166 = "$MQL5_BASE\EA - Budak Ubat v1.66 - MT5 - .ex5"
        if (Test-Path $bin166) {
            Copy-Item $bin166 "$MQL5_BASE\EA - Budak Ubat v1.66 - MT5 - $dateSuffix.ex5" -Force
            Copy-Item $bin166 "$PUBLIC_REPO\EA - Budak Ubat v1.66 - MT5 - $dateSuffix.ex5" -Force
            Copy-Item $bin166 "$PUBLIC_REPO\EA - Budak Ubat v1.66 - MT5 - .ex5" -Force
            Copy-Item $bin166 "$DESKTOP_MT5_REPO\EA - Budak Ubat v1.66 - MT5 - $dateSuffix.ex5" -Force
            Copy-Item $bin166 "$DESKTOP_MT5_REPO\EA - Budak Ubat v1.66 - MT5 - .ex5" -Force
            Copy-Item $mq5_166 "$DESKTOP_MT5_REPO\EA - Budak Ubat v1.66 - MT5 - .mq5" -Force
            Write-Host "  [OK] EA Budak Ubat MT5 v1.66 compiled!" -ForegroundColor Green
        }
    }

    # 0.1. EA Budak Ubat MT5 v1.65
    ...
    if (Test-Path $mq5_164) {
        ...
        Start-Process -FilePath $MT5_COMPILER -ArgumentList "/compile:`"$mq5_164`" /log:`"$MQL5_BASE\compile_v164.log`"" -PassThru -Wait -NoNewWindow
        $bin164 = "$MQL5_BASE\EA - Budak Ubat v1.64 - MT5 - .ex5"
        if (Test-Path $bin164) {
            Copy-Item $bin164 "$MQL5_BASE\EA - Budak Ubat v1.64 - MT5 - $dateSuffix.ex5" -Force
            Copy-Item $bin164 "$PUBLIC_REPO\EA - Budak Ubat v1.64 - MT5 - $dateSuffix.ex5" -Force
            Copy-Item $bin164 "$DESKTOP_MT5_REPO\EA - Budak Ubat v1.64 - MT5 - $dateSuffix.ex5" -Force
            Copy-Item $bin164 "$DESKTOP_MT5_REPO\EA - Budak Ubat v1.64 - MT5 - .ex5" -Force
            Copy-Item $mq5_164 "$DESKTOP_MT5_REPO\EA - Budak Ubat v1.64 - MT5 - .mq5" -Force
            Write-Host "  [OK] EA Budak Ubat MT5 v1.64 compiled!" -ForegroundColor Green
        }
    }
